 ...
 *
 * Display is left to the OS: the server sends NOTIFICATION messages, which
 * Android renders directly even when the app is killed (data-only messages were
 * getting dropped by battery optimization — verified on-device). Foreground
 * messages are surfaced in-app by the realtime provider, so we don't pop a
...
export async function registerForPushNotifications(): Promise<void> {
  ...
  try {
    ...
    // Foreground messages are handled in-app (live chat + toast), so don't also
    // pop a system notification — that would double up and notify you about the
    // chat you're already reading.
    messaging().onMessage(async () => {});

    // Tap a notification while the app is backgrounded → open the chat.
